...
import { User, Phone, Home, UserSquare } from 'lucide-react';
  ...
  return (
    <Card className="shadow-lg">
      ...
      {(customer.idProofUrl || customer.mediatorName) && (
        <CardContent className="border-t pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          {customer.idProofUrl && (
            <div>
              <h4 className="font-semibold mb-2 text-foreground">ID Proof</h4>
              <Image
          ...
          {customer.mediatorName && (
             <div>
              <h4 className="font-semibold mb-2 text-foreground">Mediator Details</h4>
